...
func NewMindCli(robotScanner *RobotScanner, userConfig *UserConfig, portconfig *PortConfig, config *MindCliConfig) *MindCli {
...
func (mindcli *MindCli) RunSkill(noInstall bool, robotIp string, args ...string) {
	if robotIp == "" {
		...
		if robots[0].Name != robot.Name {
			fmt.Printf("Looks like robot %s has changed its IP, which should be %s \n Please run `mind scan` to find it out.", robot.Name, robot.IP)
			os.Exit(-1)
		}
	...
	err := mindcli.ChangePort(robotIp)
	if err != nil {
		print(err)
		os.Exit(-1)
	}
...
func (mindcli *MindCli) Scan(waitDuration int, args ...string) ([]Robot, error) {
	...
	err = mindcli.userConfig.Write()
	if err != nil {
		return robots, err
	}
	err = mindcli.AllocatePort()
	return robots, err
}
