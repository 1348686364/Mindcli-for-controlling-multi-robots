...
func main() {
	...
	portConfig := mindcli.NewPortConfig(filepath.Join(home, ".mindport.json"))
	robotScanner := mindcli.NewRobotScanner(NewDefaultRobotScannerConfig())
	cli := mindcli.NewMindCli(robotScanner, userConfig, portConfig, NewDefaultMindCliConfig())
...
func newMindCommand(cli *mindcli.MindCli) *cobra.Command {
	var mindCmd = &cobra.Command{
		Use:   "mind",
		Short: "MIND Command-line Interface v0.6.1 For Multi Control(@ZXT)",
	}
	...
	mindCmd.AddCommand(cmd.NewGroupCommand(cli))
	return mindCmd
}
